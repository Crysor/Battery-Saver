...
open class DeviceGuru {
  ...
  class open func hardwareNumber() -> Float {
    ...
    class open func hardwareDetails() -> [String: String] {
    
        var Details = [String: String]()
        let hardware = hardwareString()
        if let deviceList = getDeviceList() {
            let hardwareDetail = deviceList[hardware] as? [String: AnyObject]
            if let hardwareCPU = hardwareDetail?["CPU"] as? String {
                Details["CPU"] = hardwareCPU
            }
            if let hardwareWeight = hardwareDetail?["weight"] as? String {
                Details["weight"] = hardwareWeight
            }
            if let hardwareSZ = hardwareDetail?["screen_size"] as? String {
                Details["screen_size"] = hardwareSZ
            }
            if let hardwareSize = hardwareDetail?["size"] as? String {
                Details["size"] = hardwareSize
            }
            if let hardwareReso = hardwareDetail?["resolution"] as? String {
                Details["resolution"] = hardwareReso
            }
            if let hardwareCamFront = hardwareDetail?["cam_front"] as? String {
                Details["cam_front"] = hardwareCamFront
            }
            if let hardwareCam = hardwareDetail?["cam"] as? String {
                Details["cam"] = hardwareCam
            }
            if let hardwareBattery = hardwareDetail?["battery"] as? String {
                Details["battery"] = hardwareBattery
            }
        }
